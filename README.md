...
- **日期语义校验**：优先保存渠道明确公布的计划日期；首次采集发现日不会冒充首发日，并在默认列表中排除。
...
- **双维度看板**：支持“产品聚合 / 渠道明细”切换，以及今日/周/月/全部、自定义日期、事件日历、关键词及品类/厂商/渠道/事件类型筛选。
...
采集写入使用“来源 + 来源产品 ID + 事件类型 + 事件时间”的业务键保证幂等。没有明确渠道日期的事件保留首次采集发现时间，并与计划日期分别展示。
